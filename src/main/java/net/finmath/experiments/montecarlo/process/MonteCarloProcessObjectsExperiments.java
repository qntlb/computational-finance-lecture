...
//		MonteCarloProcessObjectsExperiments.testEulerSchemeWithBlackScholesModel();
		...
		MonteCarloProcessObjectsExperiments.testValuationWithProduct();
	}

	private static void testValuationWithProduct() throws CalculationException {
		/*
		...
		System.out.println("\n");
		System.out.println("Valuation with Product");
		...
		double valueMonteCarlo = product.getValue(model);
	...
	private static void testValuation() throws CalculationException {
		/*
		 * A Brownian Motion takes a TimeDiscretization and a RandomNumberGenerator
		 * and creates a set of RandomVariable-s representing the increments of
		 * a n-dimensional Brownian motion.
		...
		RandomVariable valuation = payoff
				.div(processModel.getNumeraire(process, maturity))
				.mult(processModel.getNumeraire(process, initialTime));
		
		double valueMonteCarlo = valuation.getAverage();
		double valueAnalytic = AnalyticFormulas.blackScholesOptionValue(initialValue, riskFreeRate, volatility, maturity, strike);
